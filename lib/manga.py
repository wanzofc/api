...
def scrap_komiku(url):
    try:
        ...
        thumb = ko.find('div', class_='bge').img['src']
        ...
        dllink = ['https://komiku.id'+i.a['href'] for i in ko.findAll('td', class_='tanggalseries dl')]
...
def search_komiku(query):
    try:
        url = bs(get('https://data.komiku.id/cari/?post_type=manga&s=%s' % query).text, 'html.parser').find('div', class_='bge').a['href']
        return url
    except:
