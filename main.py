...
def check_limit():
    if request.args.get('apikey'):
        if request.args.get('apikey') in keyMe:
            ...
            return {
                'status': False,
                'message': 'APIKEY LU GA VALID TOD'
            }
    else:
        return {
            'status': False,
            'message': 'MASUKIN APIKEY LU'
        }
		...
		if theme.lower() in tp.theme:
			if theme.lower() == 'glitch':
				if request.args.get('apikey') in keyMe:
					...
					return result
				else:return {'creator': 'Tobz','status': False,'message': 'APIKEY LU GA VALID TOD'}
	...
	if request.args.get('text'):
		if request.args.get('apikey') in keyMe:
			try:
				...
				if keyMe[kekeyi]['limit'] < 1: return {'status': False,'error': 'Apikey has reached the limit'}
